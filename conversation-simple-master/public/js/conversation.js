var ConversationPanel = (function() {// The ConversationPanel module is designed to handle
  ...
  function setupInputBox() {// Set up the input box to underline text as it is typed
    ...
    function adjustInput() {// If the input box is empty, remove the underline
      if (input.value === '') {
        input.classList.remove('underline');
        input.setAttribute('style', 'width:' + '100%');
        input.style.width = '100%';
      }
      else {// otherwise, adjust the dummy text to match, and then set the width of the visible input box to match it (thus extending the underline)
  ...
  function displayMessage(newPayload, typeValue) {// Display a user or Watson message that has just been sent/received
    ...
    if (isUser !== null && textExists) {// Create new message DOM element
